...
const fake = '© Welcome Message By @ryzuubotz'
const fakee = '© Leave Message By @ryzuubotz'
...
- ${metadata.subject} -
│
╰─ Dont Comeback Here Again >.<`
let Tod = `⎗ㅤㅤ⎚ㅤㅤ⎘ㅤㅤ⎌
Wеᥣcоmе tо Group ${metadata.subject}\nJgn Lupa Intro @${num.split('@')[0]}👋🏻\n◌⃘  Nama :
◌⃘  Umur :
◌⃘  Askot :
◌⃘  Kelamin :
╰─ Dont forget to obey the rules in the group >.<`
          ...
          },
          {
            "quickReplyButton": {
              ...
              "id": `Welcome`
                        ...
                        celinecans = await getBuffer(ppuser)
                      ezty.send5Loc(anu.id, Tod, fake, celinecans, btn)
...
ezty.sendMessage(anu.id, { audio: ini_buffer, mimetype: 'audio/mp4', ptt: true })
          ...
          {
            "quickReplyButton": {
              "displayText": "Sayonara Jlk 👋🏻",
              "id": `Sayonara`
            }
        }
        ]
                        celinecans = await getBuffer(ppuser)
                      ezty.send5Loc(anu.id, Ttes, fakee, celinecans, btn)
